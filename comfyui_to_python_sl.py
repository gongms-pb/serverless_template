...
class CodeGenerator:
    ...
    def assemble_python_code(
        ...
        if custom_nodes:
            ...
            static_imports.append(custom_nodes)
        else:
            custom_nodes = ""
        ...
        # Add model load comments
        model_loader = (
            [
                "\n# EDIT START : Move model loading codes from main function to here",
                "loader = None",
                "# EDIT END"
            ]
        )
        ...
        main_function_code = (
            ...
            + f"with torch.inference_mode():\n\t\t"
            + "# EDIT START : Edit inputs to given text, image, etc.\n\t\t"
            + "\n\t\t".join(speical_functions_code)
            + "\n\t\t# EDIT END"
            + f"\n\n\t\tfor q in range({queue_size}):\n\t\t"
            + "\n\t\t".join(code)
            + "\n\t# EDIT START : Edit output variable name (Assumes output image is saved with 'SaveImage' node)"
            + "\n\treturn output"
            + "\n\t# EDIT END\n"
        )
        ...
        final_code = "\n".join(
            ...
            + imports_code
            + model_loader
            + ["", main_function_code, "", 'if __name__ == "__main__":',
               "\t# EDIT START : Initialize default inputs",
               "\tinputs = {}",
               "\t# EDIT END",
               "\toutput = main(inputs)",
               "\tprint(output)"]
